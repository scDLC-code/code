...
########################
class RNN_RNAseqClassifier:
    def __init__(self, num_classes,num_steps, batch_size=11,
                lstm_size=64, num_layers=2,level_size=260,
                grad_clip=5, train_keep_prob =0.3,
...
#            self.lstm_inputs = self.inputs
            self.lstm_inputs = tf.layers.dense(self.inputs,units=200,activation=tf.nn.relu,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.003))
#            self.lstm_inputs = tf.layers.dense(self.inputs_1,units=32,activation=tf.nn.relu,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.003))

          #dense1 = tf.layers.dense(inputs=pool3, units=1024, activation=tf.nn.relu，kernel_regularizer=tf.contrib.layers.l2_regularizer(0.003))
#            if self.use_embedding is False:
#                self.lstm_inputs = tf.one_hot(self.inputs, self.level_size)
#            else:
#                with tf.device("/cpu:0"):
#                    embedding = tf.get_variable('embedding', [self.level_size, self.embedding_size])
...
#             self.output = tf.layers.dense(self.final_state[0],units=8,activation=tf.nn.relu,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.003))
#             self.logits = tf.layers.dense(self.output, self.num_classes)
             self.logits = tf.layers.dense(self.final_state[1].h, self.num_classes)

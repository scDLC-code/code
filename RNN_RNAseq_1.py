# -*- coding: utf-8 -*-
"""
Created on Sun Nov 24 22:36:28 2019
...
class RNN_RNAseqClassifier_1:
    ...
    def build_loss(self):
        ...
        with tf.name_scope('loss'):
            ...
            regularization_cost = 0.001* tf.reduce_sum([ tf.nn.l2_loss(v) for v in self.tvars ])
            original_cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=self.targets))
            self.loss= regularization_cost + original_cost
    ...
    def build_optimizer(self):
        ...
        grads, _ = tf.clip_by_global_norm(tf.gradients(self.loss,self.tvars), self.grad_clip)
        train_op = tf.train.AdamOptimizer(self.lr)
        self.optimizer = train_op.apply_gradients(zip(grads,self.tvars))
            ...
            for epoch in range(n_epoch):            
                ...
                if val_data is not None:
                    ...
                    for (X_test_batch, Y_test_batch) in self.a_batch(X=val_data[0], batch_size=self.batch_size,Y=val_data[1]):
                        v_loss, v_acc = sess.run([self.loss, self.acc],feed_dict={
